...
auto sortAlpha = [](const Score& first, const Score& last) //predicate for sort take 2 monster bits sort names
{
	return  first.get_name() < last.get_name(); //compare first name to second smallest (a) to largest (z)   hence <
};

auto sortPoints = [](const Score& first, const Score& last) //predicate for points
{
	return  first.get_points() > last.get_points(); //compare first Points to second largest (99999999) to smallest (-999999) hence >
};

auto sortTime = [](const Score& first, const Score& last) //predicate for time
{
